        ...
        <v-app-bar class="px-0" color="blue" dense dark app fixed clipped-left extension-height="64">
            <template v-if="btn_search && $vuetify.breakpoint.xs" #extension>
                <v-container fluid>
                    <v-form @submit.prevent="do_serach">
                        <v-row>
                            <v-col cols="9">
                                <v-text-field class="ma-0 pa-0" hide-details single-line solo-inverted v-model="search"
                                    ref="mobile_search"></v-text-field>
                            </v-col>
                            <v-col cols="3">
            ...
            <!-- <template v-if="$vuetify.breakpoint.smAndUp">
                <v-text-field flat solo-inverted hide-details prepend-inner-icon="search" @keyup.enter="do_search"
                    ref="search" v-model="search" name="name" label="Search" class="d-none d-sm-flex ml-8">
                </v-text-field>
                <v-spacer></v-spacer>
            </template>
            v-else -->
            <v-btn icon class="d-flex d-sm-none" @click="btn_search = !btn_search">
